# rails g model Prestamos fecha_ini:date fecha_fin:date devuelto:boolean ejemplar:references usuario:references
class PrestamosController < ApplicationController
        def index
        if params[:id].present?
            @prestamo= Prestamo.where(id: params[:id])
        else
            @prstamo = Prestamo.all
        end
        
        render json: @ejemplar
    end
    
    def create
        @ejemplar = Ejemplar.new(ejemplar_params)
        if @ejemplar.save
            render json: @ejemplar, status: :created
        else
            render json: @ejemplar.errors, status: :unprocessable_entity
        end
    end

    def update
        @ejemplar = Ejemplar.find(params[:id])
        if @ejemplar.update(ejemplar_params)
            render json: @ejemplar
        else
            render json: @ejemplar.errors, status: :unprocessable_entity
        end
    end

    def destroy
        @ejemplar = Ejemplar.find(params[:id])
        if @ejemplar.destroy
            head :ok
        else
            render json: { error: "No se pudo eliminar" }, status: :unprocessable_entity
        end
    end

    private

    def usuario_params
        params.require(:ejemplar).permit(:producto, :disponible)
    end
end
